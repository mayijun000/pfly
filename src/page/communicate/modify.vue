...
<script>
	import { getCommunicateInfo, getCommunicateSave, getMemberSelect, getLinkManSelect } from '@/api/getData'
		...
		methods: {
			...
			async getCommunicateInfoData() {
				try {
					...
					if(returnData.result == "success") {
						...
						this.formInline.communicationId = item.communicationId;
						this.formInline.communicationDate = item.communicationDate;
						this.formInline.contactsId = item.contactsId;
						this.formInline.memberId = item.memberId;
						this.formInline.content = item.content;
					} else {
						this.$message({
							type: 'error',
							message: returnData.message
			...
			saveCommunicate(formInline) {
				this.$refs[formInline].validate(async(valid) => {
					if(valid) {
						try {
							if(typeof(this.formInline.communicationDate) != "string") {
								this.formInline.communicationDate = this.formInline.communicationDate.format('yyyy-MM-dd');
							}
							let data = {
								memberId: this.formInline.memberId,
								salerId: this.user.userId,
								contactsId: this.formInline.contactsId,
								communicationDate: this.formInline.communicationDate,
