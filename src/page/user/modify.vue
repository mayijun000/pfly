...
<script>
	import { getUserInfo, getUserSave, getRoleList } from '@/api/getData'
	export default {
		data() {
			...
			async initData() {
				try {
					...
					if(returnData.result == "success") {
						...
						this.formData.mobile = returnData.obj.mobile;
						this.formData.userName = returnData.obj.userName;
						this.formData.userId = returnData.obj.userId;
						returnData.roles.forEach(item => {
							role.forEach(data => {
								if(item.roleId == data.roleId) {
									obj.push(item.roleId);
								}
							});
						});
						this.formData.roleId = obj;
