...
<script>
	import { getClassification, getRequirementSave, queryProductExistence, getRequirementInfo } from '@/api/getData';
	import { getStore, getWeekDate, getMonthDate } from '@/config/mUtils'
	export default {
		data() {
				...
				try {
					...
					if(returnData.result == "success") {
						...
						if(item.month == "" && item.week != "") {
							this.formData.requirementType = "3";
							this.formData.week = getWeekDate(item.year, item.week);
						} else if(item.week == "" && item.month != "") {
							this.formData.requirementType = "2";
							this.formData.month = getMonthDate(item.year, item.month);
						} else {
							this.formData.requirementType = "1";
