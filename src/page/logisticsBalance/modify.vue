							...
							<table cellspacing="0" cellpadding="0" border="0" class="el-table__body" style="width:100%;">
								...
								<tbody>
									<tr v-for="item in selectProductDataList" class="el-table__row">
										...
										<td :rowspan="item.deliveryNumspan" :class="{hidden: item.deliveryNumdis}">
											<div class="cell">
												<el-input placeholder="请输入运费" style="width:93px;" prop="total" v-model="item.total" @change="changPrice" type=""></el-input>
											</div>
										</td>
									</tr>
								</tbody>
				...
				<el-card style="margin-top: 20px;">
					<el-form :model="findForm" :inline="true" class="demo-form-inline">
						...
						<el-form-item label="物流公司">
							<el-select v-model="findForm.logisticsId" clearable placeholder="请选择物流公司">
								...
								<table cellspacing="0" cellpadding="0" border="0" class="el-table__body" style="width:100%;">
									...
									<tbody>
										<tr prop="productDataList" v-for="item in findFormReg.productDataList" class="el-table__row">
											...
											<td :rowspan="item.deliveryNumspan" :class="{hidden: item.deliveryNumdis}">
												<div class="cell">
													<el-input placeholder="" style="width:93px;" prop="total" v-model="item.total"></el-input>
												</div>
											</td>
										</tr>
									</tbody>
...
<script>
	...
	import { balanceInfo, balanceSave, getMemberSelect } from '@/api/getData'
	export default {
		data() {
			return {
		...
		methods: {
				...
				try {
					...
					if(stockData.result == "success") {
						...
						stockData.obj.itemList.forEach((item, index) => {
							...
							totalFee = parseFloat(totalFee) + parseFloat(item.total);
						...
						//复杂表格数据处理
						this.selectProductDataList = combineCell(this.tableData);
			...
			changPrice() {
				...
				this.totalFeeEnd = total.toFixed(2);
			},
			async findData() {
				try {
					if(this.findForm.date == "" && this.findForm.logisticsId == "" && this.findForm.deliveryNum == "") {
						this.$message({
							type: 'warning',
							message: '请输入查询条件'
						});
				...
				this.selectProductDataList.forEach((data, index) => {
					if(!item.selectedStatus) {
						if(data.deliveryNum == item.deliveryNum) {
							...
							if(item.total != "") {
								this.totalFeeEnd = (parseFloat(this.totalFeeEnd) - parseFloat(item.total)).toFixed(2);
							}
							...
							for(let i = 1; i < Number(data.deliveryNumspan); i++) { //判断合并的行数
								...
								obj.selected = false;
								if(item.total != "") {
									this.totalFeeEnd = (parseFloat(this.totalFeeEnd) - parseFloat(obj.total)).toFixed(2);
								}
				...
				this.$refs[tableData].validate(async(valid) => {
					if(valid) {
						...
						if(this.selectProductDataList.length == 0) {
							...
							this.selectProductDataList.forEach(item => {
								if(item.total == "") {
									this.$message({
										message: '请检查是否输入了运费',
										type: 'warning'
									...
									return false;
								} else if(!reg.test(item.total)) {
									this.$message({
										message: '请检查输入的运费是否是整数或小数点两位',
										type: 'warning'
									});
									return false;
								} else {
				...
				try {
					...
					this.selectProductDataList.forEach((item, index) => {
						...
						if(item.selected) {
							...
							itemList1.total = item.total
							itemList1.deliverId = item.deliverId
							itemList1.balanceId = item.balanceId
