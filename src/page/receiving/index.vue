		...
		<el-row>
			<el-col :span="24">
				...
				<el-table :data="tableData" border stripe style="width: 100%">
					...
					<el-table-column prop="_driverId" label="司机" width="80">
					</el-table-column>
					<el-table-column prop="carNum" label="车牌号" min-width="100">
					</el-table-column>
					<el-table-column prop="realDeliverTotal" label="实发总量" min-width="100">
					</el-table-column>
					<el-table-column prop="receiptNumber" label="收货总量" min-width="100">
					</el-table-column>
					<el-table-column prop="_addressId" label="收货地址" min-width="100">
					</el-table-column>
					<el-table-column prop="statusName" label="状态" width="80">
					</el-table-column>
					<el-table-column label="操作" width="120">
						<template scope="scope">
							<el-button v-show="scope.row.showBtn" size="small" type="warning" @click="detail(scope.row)">填写收货单</el-button>
						</template>
					</el-table-column>
				</el-table>
				<div class="block">
...
<script>
	import { getReceiveList, getDriverList } from '@/api/getData'
	export default {
		data() {
			return {
				findForm: {
		...
		methods: {
			async getDriverListData() {
				try {
					...
					const stockData = await getDriverList({
						page: page,
					});
					if(stockData.result == "success") {
						this.driverList = stockData.list;
			...
			async initData() {
				try {
					...
					const stockData = await getReceiveList({
