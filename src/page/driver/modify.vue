...
<script>
	import { getDriverSave, getMemberSelect, getDriverInfo } from '@/api/getData'
	export default {
		data() {
			...
			async getDriverInfoData() {
				try {
					...
					if(returnData.result == "success") {
						...
						this.driverData.memberId = item.memberId;
						this.driverData.driverId = item.driverId;
						this.driverData.carNum = item.carNum;
						this.driverData.userName = item.userName;
						this.driverData.mobile = item.mobile;
					} else {
						this.$message({
							type: 'error',
							message: returnData.message
