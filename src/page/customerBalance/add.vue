						...
						<div class="el-table__header-wrapper">
							<table cellspacing="0" cellpadding="0" border="0" class="el-table__header" style="width:100%;">
								<colgroup>
									...
									<col name="el-table_1_column_1" width="145">
								...
								<tbody>
									<tr v-for="item in selectProductDataList" class="el-table__row">
										<td :rowspan="item.deliveryNumspan" :class="{hidden: item.deliveryNumdis}">
											<div class="cell">
												<el-checkbox v-model='item.selected' @change="selectProduct(item)"></el-checkbox>
											</div>
										</td>
										<td :rowspan="item.deliveryNumspan" :class="{hidden: item.deliveryNumdis}">
										...
										<td>
											<div class="cell">
												<el-input prop="price" v-model="item.price" @change="changPrice" type=""></el-input>
											</div>
										</td>
										<td>
				...
				<el-card style="margin-top: 20px;">
					<el-form :model="findForm" :inline="true" class="demo-form-inline">
						...
						<el-form-item label="客户">
							<el-select v-model="findForm.supplierId" clearable placeholder="请选择客户">
								...
								<table cellspacing="0" cellpadding="0" border="0" class="el-table__body" style="width:100%;">
									...
									<tbody>
										<tr prop="productDataList" v-for="item in findFormReg.productDataList" class="el-table__row">
											...
											<td>
												<div class="cell">
													<el-input prop="price" v-model="item.price" type=""></el-input>
												</div>
											</td>
											<td>
												<div class="cell">{{item.balancePrice}}</div>
...
<script>
	...
	import { balanceList, balanceSave, getMemberSelect } from '@/api/getData'
	export default {
		data() {
			return {
		...
		methods: {
			...
			async initData() {
				...
				try {
					if(this.findForm.date == "" && this.findForm.demandId == "" && this.findForm.deliveryNum == "") {
						this.$message({
							type: 'warning',
							message: '请输入查询条件'
						});
			...
			changPrice() {
				...
				this.selectProductDataList.forEach(data => {
					if(data.price != "") {
						total += parseFloat(data.price) * parseFloat(data.receiveNumber);
					}
				});
